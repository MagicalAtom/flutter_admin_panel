...
class SwiftRequest implements SwiftRequestInterFace {
  ...
  Future get({required String path, queryParam}) async {
    try {
      Response response = await DioService.get(
          path, queryParameters: queryParam);
      return response;
  ...
  Future delete({required path, required id}) async {
    try {
      Response response = await DioService.delete("$path/$id");
      return response;
    } on DioException catch (e) {
      throw HttpExceptionHandle(
        statusCode: e.response?.statusCode,
  ...
  @override
  Future put({required path, required id, required data}) async {
    try {
      Response response = await DioService.put("$path/$id",data: data);
      return response;
    } on DioException catch (e) {
      throw HttpExceptionHandle(
        statusCode: e.response?.statusCode,
